...
1. In the same directory as the last exercise (still logged into `training.osgconnect.net`) make a file called `run_blast.sh`. 
...
1. Opening your `run_blast.sh` script, add two commands at the start to un-tar the BLAST and pdbaa `tar.gz` files. See the [previous exercise](/materials/day2/part3-ex2-precompiled) if you're not sure what this command looks like. 
